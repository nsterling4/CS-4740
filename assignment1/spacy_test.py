...
def ngram_prob (filename):
	...
	for fn in os.listdir(filename):
	    ...
	    if not fn.startswith('.'):
	    	corpus = corpus + tokenize(filename + fn, en_nlp)
...
ngram_prob ('data_corrected/classification task/space/train_docs')
...
def spellcheck (filename):
	biprob= ngram_prob (filename)




spellcheck('data_corrected/spell_checking_task/atheism/')
	


for fn in os.listdir('data_corrected/spell_checking_task/'):
    if os.path.isdir(fn):
    	for file in ('data_corrected/spell_checking_task/'+fn+"train_docs"):
